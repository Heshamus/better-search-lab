...
/**
 * Records a seeding failure through the same status path `ensureDemoSeeded`
 * uses internally. Exported so `instrumentation.ts`'s own outer try/catch
 * can record a failure it caught OUTSIDE `ensureDemoSeeded` (e.g. one of its
 * own dynamic imports failing to resolve) without duplicating the write.
 */
export function recordDemoSeedFailure(error: string): void {
