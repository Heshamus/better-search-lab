  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="panel overflow-x-auto">
        <div className="flex items-baseline justify-between gap-2 border-b border-neutral-200 px-4 py-2.5">
          <h3 className="text-sm font-semibold text-neutral-900">Rising queries</h3>
        ...
        <table className="w-full min-w-[360px] border-collapse text-left text-sm">
          ...
          <tbody>
            {data.risingQueries.length ? data.risingQueries.map((r) => (
              <tr key={r.query} className="border-b border-neutral-200 transition-colors last:border-0 hover:bg-neutral-50">
                <td className="px-4 py-2.5 font-medium text-neutral-900">{r.query}</td>
                <td className="px-4 py-2.5">
                  {r.page ? (
                    <span className="text-neutral-700" title={r.page}>{shortPath(r.page)}</span>
                  ) : (
                    <span className="rounded bg-[--color-at-risk-tint] px-1.5 py-0.5 text-[0.66rem] font-medium text-at-risk">no page · gap</span>
                  )}
                </td>
                <td className="px-4 py-2.5"><span className="tnum text-neutral-800">{formatCompact(r.recent)}</span></td>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="panel overflow-x-auto">
          <div className="border-b border-neutral-200 px-4 py-2.5"><h3 className="text-sm font-semibold text-neutral-900">Top queries</h3></div>
          <table className="w-full min-w-[360px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-neutral-200">
                <th className="eyebrow px-4 py-2.5">Query</th>
            ...
            <tbody>
              {data.topQueries.length ? data.topQueries.map((r) => (
                <tr key={r.key} className="border-b border-neutral-200 transition-colors last:border-0 hover:bg-neutral-50">
                  <td className="px-4 py-2.5 font-medium text-neutral-900">{r.key}</td>
                  <td className="px-4 py-2.5"><span className="tnum text-neutral-800">{r.clicks}</span></td>
                  <td className="px-4 py-2.5"><span className="tnum text-neutral-600">{formatCompact(r.impressions)}</span></td>
                  <td className="px-4 py-2.5"><span className="tnum text-neutral-700">{r.position.toFixed(1)}</span></td>
                </tr>
        ...
        <div className="panel overflow-x-auto">
          <div className="border-b border-neutral-200 px-4 py-2.5"><h3 className="text-sm font-semibold text-neutral-900">Top pages</h3></div>
          <table className="w-full min-w-[360px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-neutral-200">
            ...
            <tbody>
              {data.topPages.length ? data.topPages.map((r) => (
                <tr key={r.key} className="border-b border-neutral-200 transition-colors last:border-0 hover:bg-neutral-50">
                  <td className="px-4 py-2.5 font-medium text-neutral-900" title={r.key}>{shortPath(r.key)}</td>
                  <td className="px-4 py-2.5"><span className="tnum text-neutral-800">{r.clicks}</span></td>
                  <td className="px-4 py-2.5"><span className="tnum text-neutral-600">{formatCompact(r.impressions)}</span></td>
                  <td className="px-4 py-2.5"><span className="tnum text-neutral-700">{r.position.toFixed(1)}</span></td>
                </tr>
