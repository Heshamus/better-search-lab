...
export function EmptyState({ title, description, action }: { title: string; description: string; action?: React.ReactNode }) {
  return (
    <section className="panel flex flex-col items-center px-6 py-16 text-center">
      <div aria-hidden className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-neutral-100 text-neutral-500">
      ...
      <p className="mx-auto mt-1.5 max-w-sm text-sm text-neutral-600">{description}</p>
...
export function IntegrationLink({ group, label }: { group: string; label: string }) {
  return (
    <a href={`/settings/integrations#${group}`} className="rounded-lg bg-neutral-900 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-[#2a3138]">
      {label}
    </a>
  );
}
