#!/bin/sh
# Better Search Lab installer. Docker (with Compose v2) and curl are the only requirements.
#
...
# The whole installer lives in a function called on the last line, so a download
# truncated mid-transfer runs nothing at all.
main() {
  MODE=app
  for arg in "$@"; do
    case "$arg" in
  ...
  if [ -n "${BSL_COMPOSE_FILE:-}" ]; then
    ...
    curl -fsSL "$RAW/$FILE" -o "$DIR/$FILE.tmp"
  ...
  if [ "$MODE" = app ] && [ ! -f .env ]; then
    ...
    [ -n "$SECRET" ] || { echo "Could not generate AUTH_SECRET: install openssl and run again." >&2; exit 1; }
